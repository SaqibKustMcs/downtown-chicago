import 'package:tabler_icons/tabler_icons.dart';

/// Helper class for Tabler Icons
class TablerIconsHelper {
  TablerIconsHelper._();

  // Navigation & UI
  static const menu = TablerIcons.menu_2;
  static const apps = TablerIcons.apps;
  static const search = TablerIcons.search;
  static const arrowDown = TablerIcons.chevron_down;
  static const arrowRight = TablerIcons.chevron_right;
  static const arrowLeft = TablerIcons.chevron_left;
  static const close = TablerIcons.x;

  // Shopping & Cart
  static const shoppingBag = TablerIcons.shopping_bag;
  static const shoppingCart = TablerIcons.shopping_cart;

  // Restaurant & Food
  static const restaurant = TablerIcons.tools_kitchen;
  static const star = TablerIcons.star;
  static const delivery = TablerIcons.truck_delivery;
  static const time = TablerIcons.clock;

  // User & Profile
  static const person = TablerIcons.user;
  static const personOutline = TablerIcons.user_circle;

  // Orders
  static const receipt = TablerIcons.receipt;

      // Profile & Settings
      static const location = TablerIcons.map_pin;
      static const payment = TablerIcons.credit_card;
      static const creditCard = TablerIcons.credit_card;
  static const favorite = TablerIcons.heart;
  static const settings = TablerIcons.settings;
  static const help = TablerIcons.help;
  static const logout = TablerIcons.logout;
  static const chevronRight = TablerIcons.chevron_right;

  // Common
  static const home = TablerIcons.home;
  static const homeOutline = TablerIcons.home_2;
  
  // Additional icons
  static const chevronDown = TablerIcons.chevron_down;
  static const filter = TablerIcons.filter;
  static const plus = TablerIcons.plus;
  static const minus = TablerIcons.minus;
  static const truck = TablerIcons.truck;
  static const clock = TablerIcons.clock;
  static const bell = TablerIcons.bell;
  static const edit = TablerIcons.pencil;
  static const check = TablerIcons.check;
  static const trash = TablerIcons.trash;
  
  // Theme
  static const sun = TablerIcons.sun;
  static const moon = TablerIcons.moon;
}
